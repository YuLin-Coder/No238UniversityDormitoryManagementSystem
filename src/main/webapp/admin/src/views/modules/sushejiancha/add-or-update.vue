...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                                                                  ...
                                                                                                                  this.jianchajieguoOptions = "良好,合格,不合格".split(',')
                                        this.pingfenOptions = "1,2,3,4,5".split(',')
                                                                                  },
                                                                                // 多级联动参数
                                                                                                      info(id) {
      this.$http({
        url: `sushejiancha/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `sushejiancha/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.sushejianchaCrossAddOrUpdateFlag = false;
                  this.parent.search();
